...
// ==========================================
// 🛡️ EMERGENCY RECOVERY & SAFE MODE MECHANISM
// ==========================================
// This global error handler captures any runtime crashes during initialization or sync
// and presents a "Safe Mode" UI allowing the user to copy their local data and reset the app.
window.addEventListener('error', function(event) {
  console.error("Global captured error:", event.error);
  showEmergencyRecoveryUI(event.error);
});
window.addEventListener('unhandledrejection', function(event) {
  console.error("Global captured promise rejection:", event.reason);
  showEmergencyRecoveryUI(event.reason);
});

function showEmergencyRecoveryUI(error) {
  try {
    let overlay = document.getElementById('emergencyRecoveryOverlay');
    if (!overlay && document.body) {
      overlay = document.createElement('div');
      overlay.id = 'emergencyRecoveryOverlay';
      overlay.style.cssText = 'position:fixed; top:0; left:0; width:100vw; height:100vh; background-color:rgba(15,23,42,0.95); z-index:999999; display:flex; flex-direction:column; align-items:center; justify-content:center; padding:20px; color:#f8fafc; font-family:system-ui,-apple-system,sans-serif; text-align:center;';
      
      const container = document.createElement('div');
      container.style.cssText = 'max-width:500px; background-color:#1e293b; border-radius:16px; padding:30px; box-shadow:0 25px 50px -12px rgba(0,0,0,0.5); border:1px solid #334155;';
      
      const errMsg = error ? (error.stack || error.message || error) : 'Unknown Runtime Error';
      
      container.innerHTML = `
        <div style="font-size: 48px; margin-bottom: 16px;">🛡️</div>
        <h3 style="font-size: 20px; font-weight: 700; margin-bottom: 12px; color: #f1f5f9;">系统运行异常 (Safe Mode)</h3>
        <p style="font-size: 14px; color: #94a3b8; margin-bottom: 20px; line-height: 1.6;">
          很抱歉，程序在运行或同步时遇到了异常。为了防止您的减重数据丢失，已自动为您开启安全修复模式。
        </p>
        <div style="background-color: #0f172a; padding: 12px; border-radius: 8px; font-family: monospace; font-size: 11px; color: #f43f5e; text-align: left; max-height: 120px; overflow-y: auto; margin-bottom: 24px; border: 1px solid #1e293b; word-break: break-all;">
          ${errMsg}
        </div>
        <div style="display: flex; flex-direction: column; gap: 10px;">
          <button id="emergencyReloadBtn" style="background: linear-gradient(135deg, #06b6d4, #0891b2); color: white; border: none; padding: 12px 24px; border-radius: 8px; font-weight: 600; cursor: pointer; font-size: 14px;">
            🔄 重新加载页面
          </button>
          <button id="emergencyResetBtn" style="background: #ef4444; color: white; border: none; padding: 12px 24px; border-radius: 8px; font-weight: 600; cursor: pointer; font-size: 14px;">
            ⚠️ 清除缓存并强制重置
          </button>
          <button id="emergencyBackupBtn" style="background: #334155; color: white; border: none; padding: 10px 24px; border-radius: 8px; font-weight: 600; cursor: pointer; font-size: 13px;">
            💾 导出当前本地数据备份
          </button>
        </div>
        <textarea id="emergencyBackupArea" style="display: none; width: 100%; height: 120px; margin-top: 16px; background: #0f172a; color: #94a3b8; border: 1px solid #334155; border-radius: 8px; padding: 8px; font-family: monospace; font-size: 11px;"></textarea>
      `;
      
      overlay.appendChild(container);
      document.body.appendChild(overlay);
      
      document.getElementById('emergencyReloadBtn').addEventListener('click', () => {
        window.location.reload();
      });
      
      document.getElementById('emergencyResetBtn').addEventListener('click', () => {
        if (confirm('确定要清除所有本地缓存数据吗？这将重置应用。如果有云端备份，您可以在重新登录后恢复数据。')) {
          localStorage.clear();
          window.location.reload();
        }
      });
      
      document.getElementById('emergencyBackupBtn').addEventListener('click', () => {
        const area = document.getElementById('emergencyBackupArea');
        area.style.display = 'block';
        area.value = JSON.stringify(localStorage);
        area.select();
        alert('已将本地 LocalStorage 原始数据导出至下方文本框，请双击全选复制保存！');
      });
    }
  } catch (e) {
    console.error("Failed to render emergency UI:", e);
  }
}

// 默认应用状态 (使用 Proxy 代理，进行类型防御，防范 null/undefined 写入导致的运行时崩溃)
...
function loadData() {
  ...
  if (savedState) {
    try {
      ...
      appState.profile = (parsed.profile && typeof parsed.profile === 'object') ? parsed.profile : null;
  ...
  currentSyncPromise = (async () => {
    ...
    try {
      ...
      if (localUpdatedAt > cloudUpdatedAt) {
        ...
        appState.profile = (cloudData.profile && typeof cloudData.profile === 'object') ? cloudData.profile : null;
        appState.records = (cloudData.records && typeof cloudData.records === 'object') ? cloudData.records : {};
...
function renderProfileCenterPage() {
  ...
  const cuisineLabels = {
    chinese: appState.language === 'en' ? 'Chinese' : '中餐膳食',
    american: appState.language === 'en' ? 'American' : '美式轻卡',
    japanese: appState.language === 'en' ? 'Japanese' : '和风日式'
  };
  const patternLabels = {
    standard: appState.language === 'en' ? 'Standard 3-Meals' : '标准一日三餐',
    '16_8': appState.language === 'en' ? '16+8 Fasting' : '16:8断食',
    '20_4': appState.language === 'en' ? '20:4 Fasting' : '20:4战士断食',
    '5_2': appState.language === 'en' ? '5+2 Fasting' : '5:2轻断食'
  };
  
  const recipeSeriesLabel = recipeLabels[appState.profile.recipeSeries] || appState.profile.recipeSeries || (appState.language === 'en' ? 'Water-Oil Braised' : '水油焖菜系列');
  const cuisineLabel = cuisineLabels[appState.profile.preferredCuisine] || appState.profile.preferredCuisine || (appState.language === 'en' ? 'Chinese' : '中餐');
  const dietPatternLabel = patternLabels[appState.profile.dietPattern] || patternLabels[appState.profile.dietPattern] || (appState.language === 'en' ? 'Standard 3-Meals' : '标准一日三餐');
  
  const labelHeightAgeGender = appState.language === 'en' ? 'Height / Age / Gender' : '身高 / 年龄 / 性别';
  const labelInitialWeight = appState.language === 'en' ? 'Initial Weight' : '初始体重';
  const labelCurrentWeight = appState.language === 'en' ? 'Current Weight' : '当前最新体重';
  const labelTargetWeight = appState.language === 'en' ? 'Target Weight' : '目标体重';
  const labelDailyCalories = appState.language === 'en' ? 'Daily Calorie Budget' : '每日热量预算';
  const labelFastingPattern = appState.language === 'en' ? 'Fasting Pattern' : '断食模式';
  const labelRecipeSeries = appState.language === 'en' ? 'Recommended Recipes' : '推荐食谱系列';
  
  const genderLabel = appState.profile.gender === 'female' 
    ? (appState.language === 'en' ? 'Female' : '女') 
    : (appState.language === 'en' ? 'Male' : '男');
  
  summaryEl.innerHTML = `
    <div style="display:flex; flex-direction:column; gap:8px;">
      <div style="display:flex; justify-content:space-between; border-bottom:1px solid var(--border-color); padding-bottom:6px;">
        <span style="color:var(--text-muted);">${labelHeightAgeGender}</span>
        <span style="font-weight:600; color:var(--text-main);">${appState.profile.height} cm / ${appState.profile.age} ${appState.language === 'en' ? 'yrs' : '岁'} / ${genderLabel}</span>
      </div>
      <div style="display:flex; justify-content:space-between; border-bottom:1px solid var(--border-color); padding-bottom:6px;">
        <span style="color:var(--text-muted);">${labelInitialWeight}</span>
        <span style="font-weight:600; color:var(--text-main);">${appState.profile.initialWeight} kg</span>
      </div>
      <div style="display:flex; justify-content:space-between; border-bottom:1px solid var(--border-color); padding-bottom:6px;">
        <span style="color:var(--text-muted);">${labelCurrentWeight}</span>
        <span style="font-weight:600; color:var(--primary);">${currentWeight} kg</span>
      </div>
      <div style="display:flex; justify-content:space-between; border-bottom:1px solid var(--border-color); padding-bottom:6px;">
        <span style="color:var(--text-muted);">${labelTargetWeight}</span>
        <span style="font-weight:600; color:var(--accent-blue);">${appState.profile.targetWeight} kg</span>
      </div>
      <div style="display:flex; justify-content:space-between; border-bottom:1px solid var(--border-color); padding-bottom:6px;">
        <span style="color:var(--text-muted);">${labelDailyCalories}</span>
        <span style="font-weight:600; color:var(--text-main);">${appState.profile.targetCalories} kcal</span>
      </div>
      <div style="display:flex; justify-content:space-between; border-bottom:1px solid var(--border-color); padding-bottom:6px;">
        <span style="color:var(--text-muted);">${labelFastingPattern}</span>
        <span style="font-weight:600; color:var(--text-main);">${dietPatternLabel}</span>
      </div>
      <div style="display:flex; justify-content:space-between;">
        <span style="color:var(--text-muted);">${labelRecipeSeries}</span>
...
async function submitRenameUsername() {
  ...
  try {
    ...
    if (navigator.onLine) {
      ...
      // Delete old key
      try {
        await puterKvSetWithTimeout(`easyslim_sync_user_${oldUsername}`, '');
      } catch (err) {
